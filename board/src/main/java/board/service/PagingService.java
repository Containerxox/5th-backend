...
public class PagingService {
	...
	public List<Integer> getPagingNumbers(int pageNumber, int totalPages) {
		...
		// 현재 페이지 블록
		int currentBlock = pageNumber / PAGE_LENGTH; // 예시) 1~10 페이지는 0 페이지 블록에 해당
		int startPage = currentBlock * PAGE_LENGTH;// 현재 페이지(pageNumber)는 12 -> currentBlock은 1블록 -> startPage는 1+10이니까 11으로 시작됨(previous옆에 11로 시작)
		int endPage = Math.min(startPage + PAGE_LENGTH, totalPages); // startPage가 11이면, endPage=11+10 = 21 즉, 마지막 페이지값이 21이다.
